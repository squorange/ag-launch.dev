...
$essb_cache_static_instance = ESSBCacheStaticResources::instance();

class ESSBCacheStaticResources {
	...
	private function __construct() {
		...
		global $essb_cache_static_js, $essb_cache_static;
		
		if ($essb_cache_static_js == 'true') {
			add_filter( 'print_scripts_array', array( $this, 'init_essb_cache_static_js' ) );
		}
		
		if ($essb_cache_static == 'true') {
	...
	public static function instance() {
		...
		if ( ! self::$instance )
			self::$instance = new ESSBCacheStaticResources();
	...
	function essb_cache_static_objects( &$object, $todo, $extension ) {
		...
		foreach ( $essb_cache_static_todo as $script ) {
			...
			if ($pos_easy_social === false && $pos_essb === false) { continue; }
	...
	public function async_init() {
		...
		foreach ( $wp_scripts->queue as $handle ) {
			...
			$script_relative_path = ESSBCacheStaticResources::get_asset_relative_path(
				$base_url,
...
function essb_cache_static_resolve_css_urls( $content, $object, $script ) {
	...
	$src = ESSBCacheStaticResources::get_asset_relative_path(
			$object->base_url,
			$object->registered[ $script ]->src
		);
